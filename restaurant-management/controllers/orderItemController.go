...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	lookupOrderStage := bson.D{{Key: "$lookup",
		Value: bson.D{
			{"from", "order"},
			{"localField", "order_id"},
	...
	unwindOrderStage := bson.D{{Key: "$unwind",
		Value: bson.D{
			{"path", "$order"},
			{"preserveNullAndEmptyArrays", true},
		},
	...
	lookupTableStage := bson.D{{Key: "$lookup",
		Value: bson.D{
			{"from", "table"},
	...
	unwindTableStage := bson.D{{Key: "$unwind",
		Value: bson.D{
			{"path", "$order"},
			{"preserveNullAndEmptyArrays", true},
		},
	...
	projectStage2 := bson.D{{Key: "$project",
		Value: bson.D{
			{"id", 0},
			{"payment_due", 1},
			{"total_count", 1},
